...
use crate::{
    client::SelectMode, page::SelectResultPage, DEFAULT_CHOOSE_LIST_URL, DEFAULT_TIMEOUT,
    DEFAULT_USER_AGENT, SELECT_COURSE_API_URL_PRE, SELECT_COURSE_API_URL_STARTED,
    SELECT_COURSE_PAGE_URL_PRE, SELECT_COURSE_PAGE_URL_STARTED,
};

use super::{login::LoginMethod, page::DetailsPage};
...
impl Client {
    ...
    pub fn login(&self, method: &dyn LoginMethod) -> anyhow::Result<()> {
        self.clear()?;

        method.login(&self.http_client, Arc::clone(&self.cookie_store))?;

        Ok(())
    }

    pub fn select_course<'a>(
        &self,
        mode: SelectMode<'a>,
        course_no: &str,
        ...
        let _resp = self.http_client.post(api_url).form(&params).send()?;
        // let text = resp.text()?;
    ...
    pub fn refresh_details(&self) -> anyhow::Result<DetailsPage> {
        ...
        let resp = self.http_client.get(&self.choose_list_url).send()?;
        let text = resp.text()?;
        // println!("{}", text);
...
impl<'a> SelectResult<'a> {
    pub fn result(&self) -> anyhow::Result<SelectResultPage> {
        let resp = self.http_client.get(&self.page_url).send()?;
        let text = resp.text()?;
